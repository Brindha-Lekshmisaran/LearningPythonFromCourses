...
def french2eng(sentence):
     '''
     Translate french to english using the dictionary provided.
     In the dictionary, the french words are keys and english words are values.
     '''
     #convert sentence to lowercase
     sentence = sentence.lower()
     #split the sentence param into words
     words = sentence.split()
     #check the words against the dictionary keys, and replace the words with dictionary values
     #iterate through the word and position of the words list
     for pos, item in enumerate(words):
          #if the word is also a key in the dictionary
          if item in french_dict:
               #Change the value in the list to the corresponding value from dictionary
               words[pos] = french_dict[item]
     #Now all the relevant words are replaced with english meanings. 
     #Now join the words into a sentence to return

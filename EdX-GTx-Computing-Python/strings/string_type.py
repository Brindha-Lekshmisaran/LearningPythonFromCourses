...
def string_type(myStr):
    if len(myStr) == 0:
        return "empty"
    elif len(myStr) == 1:
        return "character"
    elif (myStr.count(" ") == 0) and myStr.count(".") <= 1:
        return "word"
    #If there are spaces between words, no line break, and one or less full stops it is a sentence. (I'm checking for one or less fullstop here, because a sentence can have other end characters.)
    elif (myStr.count(" ") > 0) and (myStr.count(".") <= 1) and (myStr.count("\n") == 0):
        return "sentence"
    #If there are multiple spaces and multiple full stops, but no line break it is one paragraph
    elif (myStr.count(" ") > 0) and (myStr.count(".") > 1) and (myStr.count("\n") == 0):
        return "paragraph"
     #If there are multiple spaces and multiple full stops and many line breaks there are many paragraphs
    elif (myStr.count(" ") > 0) and (myStr.count(".") > 1) and (myStr.count("\n") > 0):
        return "page"
    else:
         return None

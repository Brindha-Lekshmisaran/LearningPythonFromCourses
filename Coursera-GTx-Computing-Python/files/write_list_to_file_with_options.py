...
def write_weird_file(filename, mylist, mode="w", sort_first = False, reverse_first = False):
     '''
     Write the list to a file, opened based on params, and write each item on a seperate line. 
     '''
     ...
     if reverse_first == True:
          ...
          print(mylist)
     for item in mylist:
          print(str(item), file=outputfile)
     outputfile.close()
...
write_weird_file("output.txt", [38, 35, 43, 52], mode = "w", sort_first = False, reverse_first = True)
